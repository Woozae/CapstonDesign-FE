  ...
  return (
    <header className="header">
      <div className="header-content">
        <h1 className="header-title" onClick={() => (window.location.href = "/")}>Spot-Right</h1>
        <div className="header-buttons">
          {/* onClick 이벤트로 navigate 호출 */}
          <button className="header-button-bookmark" onClick={() => (window.location.href = "/Bookmark")}>
            Bookmark
          </button>
          <button
          className="header-button-album"
          onClick={() => (window.location.href = "/Album")}>
          Album
        </button>
          <a href="#" className="header-icon" onClick={() => (window.location.href = "/Login")}>
...
const App = () => {
  return (
    <Router>
      ...
      {/* 페이지 전환 */}
      <Routes>
        <Route path="/" element={<BookmarkPage />} /> {/* 기본 경로 */}
