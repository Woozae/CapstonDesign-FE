...
const Mainpage = () => {
  ...
  const [isInteractionEnabled, setIsInteractionEnabled] = useState(false); // 드래그, 줌 가능 여부
    ...
    loadNaverMap(() => {
      if (window.naver && window.naver.maps) {
        ...
        naver.maps.Event.addListener(mapInstance, "zoom_changed", () => {
          ...
          setLevel(newZoomLevel);

          // function.js의 updateGeoJson 호출
          if (window.updateGeoJson) {
            window.updateGeoJson(newZoomLevel);
          }
        });
      }
    });
